...
#include "cellular_hal.h"               // For the Electron Cellular
...
#define MAX_PHONE_DIGITS        14
#define CTRL_Z                  0x1A
#define CELLULAR_TIMEOUT        10000
...
//============================================================================
// Define Variables Used for the Cellular Service:
//============================================================================
char szPhoneNumber[MAX_PHONE_DIGITS] = "15073196717"; // Phone nbr with country code
boolean smsSent              = false;   // Indicates the SMS text msg was sent
...
//============================================================================
// Set the Cellular Credentials at StartUp so its Available by setup():
//============================================================================
STARTUP(cellular_credentials_set(CELLULAR_APN, CELLULAR_USERNAME, CELLULAR_PASSWORD, NULL));
 ...
 * Name: sendSmsMessage
 * 
 * Description: Package and send the SMS text nessage through the cellular 
 *              services.
 ****************************************************************************/
int sendSmsMessage(char* pMessage) {
     ...
     * from this Event.
     **/
    Serial.println("Sending Cellular Command: " + String(szCmd) + ";");

    char szReturn[32] = "";
    Cellular.command(myCallback, szReturn, CELLULAR_TIMEOUT, "AT+CMGF=1\r\n");
    Cellular.command(myCallback, szReturn, CELLULAR_TIMEOUT, szCmd);
    Cellular.command(myCallback, szReturn, CELLULAR_TIMEOUT, pMessage);
    sprintf(szCmd, "%c", CTRL_Z);
    int retVal = Cellular.command(myCallback, szReturn, CELLULAR_TIMEOUT, szCmd);
    if( RESP_OK==retVal ) {
        Serial.println("+OK, Cellular Message Sent!");
    }
    else {
        Serial.println("+ERROR, Cellular Error sending message.");
    }
    return retVal;
}
